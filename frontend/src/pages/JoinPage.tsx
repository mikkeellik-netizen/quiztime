...
export default function JoinPage() {
  ...
  useEffect(() => {
    ...
    if (tgUser) {
      setName(tgUser.first_name + (tgUser.username ? '' : ''))
    }
  }, [])
  ...
  const handleJoin = async () => {
    ...
    try {
      // Получаем JWT (в dev-режиме без Telegram)
      let token = ''
      try {
        const initData = window.Telegram?.WebApp?.initData || ''
        const resp = await axios.post(`${API_URL}/api/auth/telegram`, { initData })
        token = resp.data.access_token
      } catch {
        // В dev-режиме без Telegram пробуем без токена
      }

      const socket = getSocket(token)

      socket.emit('join_game', { gameCode: code.trim().toUpperCase(), displayName: name.trim() })
      ...
      socket.once('joined', (data: any) => {
        ...
        setPhase('lobby')
        // Сохраняем токен для реконнекта
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-6">
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-bold text-white">QuizTime</h1>
        <p className="text-[#5a6b8a] mt-2">Интерактивные викторины в реальном времени</p>
      </div>
