...
# Типы: ОДИН, НЕСКОЛЬКО, ДА-НЕТ, СВОЙ
# Для типа СВОЙ участник вводит ответ сам — в ВАРИАНТ_* перечислите принятые ответы
#   (синонимы, англ. написание и т.д.); поле ПРАВИЛЬНЫЙ не нужно.
...
---
РАУНД: Финал
ВОПРОС: Как называется столица Великобритании?
ТИП: СВОЙ
ВАРИАНТ_1: Лондон
ВАРИАНТ_2: London
ВРЕМЯ: 30
ОЧКИ: 200
ОБЪЯСНЕНИЕ: Участник вводит ответ сам, засчитываются оба варианта
---`;
...
export class ImportService {
  async generateExcelTemplate(): Promise<Buffer> {
    ...
    // Строка 1: название квиза
    const titleLabel = ws.getCell('A1');
    titleLabel.value = 'Название квиза:';
    titleLabel.font = { bold: true };
    ws.getCell('B1').value = 'Мой квиз';
    ws.getCell('B1').font = { italic: true, color: { argb: 'FF7C6DED' } };
    ...
    // Комментарий к колонке "Правильный" (теперь в строке 2)
    ws.getCell('H2').note =
      'Для ОДИН: номер варианта (1-4). Для НЕСКОЛЬКО: номера через запятую (1,2,4). ' +
      'Для ДА-НЕТ: ДА или НЕТ. Для СВОЙ: оставьте пустым — принятые ответы пишите в Вариант_1..4';

    // Выпадающий список для колонки Тип (C), строки данных 3-200
    for (let r = 3; r <= 200; r++) {
      ws.getCell(r, 3).dataValidation = {
        type: 'list',
        allowBlank: true,
        formulae: ['"ОДИН,НЕСКОЛЬКО,ДА-НЕТ,СВОЙ"'],
      };
    }
    ...
    ws.addRow(['Финал', 'Столица Великобритании?', 'СВОЙ', 'Лондон', 'London', '', '', '', '30', '200', 'Участник вводит ответ сам — засчитываются оба варианта']);
